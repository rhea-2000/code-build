            ...
            steps {
                 ...
                 script {
                     updateContainerDefinitionJsonWithImageVersion()
                      sh ("/usr/local/bin/aws ecs register-task-definition --region ${us-east-1} --family ${python-app-demo} --execution-role-arn ${IAM-role} --requires-compatibilities ${FARGATE} --network-mode ${awsvpc} --cpu ${256} --memory ${512} --container-definitions file://${./ecs/container-definition-update-image.json}")
